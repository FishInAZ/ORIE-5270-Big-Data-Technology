...
OUTPUT_FILE = "factor_model_ridge_output_daily.csv"
COEF_FILE = "factor_model_ridge_coefficients_daily.csv"
...
def rolling_ridge_prediction(work_df: pd.DataFrame, factor_cols: list, window_days: int, alpha: float):
    """
    Rolling Ridge:
    For each day t:
      - use past window_days trading days as training sample
      - estimate Ridge on ret_next ~ factors
    ...
    for i in range(window_days, len(all_dates)):
        ...
        try:
            ...
            test_data["pred_ret_ridge"] = y_pred

            pred_list.append(
                test_data[["company", "date", "ret_next", "pred_ret_ridge"]]
            )
            ...
            print(
                f"[Ridge warning] date {pred_date} skipped due to error: {e}")
            continue
...
def main():
    print_line()
    print("Daily Ridge Factor Model Pipeline Started")
    print_line()
    ...
    # 11. Rolling Ridge
    print_line()
    print(f"Running rolling Ridge with alpha = {RIDGE_ALPHA} ...")
    pred_ridge, coef_ridge = rolling_ridge_prediction(
        work_df=work_df,
    ...
    print(f"Prediction output shape: {pred_ridge.shape}")
    print(f"Coefficient output shape: {coef_ridge.shape}")

    if len(pred_ridge) > 0:
        print_line()
        print("Prediction preview:")
        print(pred_ridge.head(10))

    if len(coef_ridge) > 0:
        print_line()
        print("Coefficient preview:")
        print(coef_ridge.head(5))
    ...
    if len(pred_ridge) > 0:
        df_out = df.merge(
            pred_ridge[["company", "date", "pred_ret_ridge"]],
            on=["company", "date"],
            how="left"
        )
    else:
        df_out = df.copy()
        df_out["pred_ret_ridge"] = np.nan

    # 13. Final output
    final_cols = [
        "company", "date", "adjclose_y",
        "ret", "ret_next", "pred_ret_ridge"
    ] + FACTOR_COLS
    ...
    print(final_output[["ret_next", "pred_ret_ridge"]].describe())

    print_line()
    print("Non-null Ridge predictions:")
    print(final_output["pred_ret_ridge"].notna().sum())

    # 14. Save outputs
    final_output.to_csv(OUTPUT_FILE, index=False)
    coef_ridge.to_csv(COEF_FILE, index=False)
