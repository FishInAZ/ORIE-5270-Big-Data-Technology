...
from ridge_daily_factor_model import rolling_ridge_prediction


class TestRidgeDailyFactorModel(unittest.TestCase):
    ...
    def test_rolling_ridge_runs(self):
        """
        Test model runs
        """
        pred, coef = rolling_ridge_prediction(
            self.df,
            self.factor_cols,
            window_days=50,
            alpha=1.0
    ...
    def test_prediction_column_exists(self):
        ...
        """
        pred, _ = rolling_ridge_prediction(
            self.df,
            self.factor_cols,
            window_days=50,
            alpha=1.0
        ...
        self.assertIn("pred_ret_ridge", pred.columns)
    ...
    def test_prediction_not_all_nan(self):
        ...
        self.assertTrue(pred["pred_ret_ridge"].notna().sum() > 0)
    ...
    def test_coef_output(self):
        ...
        """
        _, coef = rolling_ridge_prediction(
            self.df,
            self.factor_cols,
            window_days=50,
            alpha=1.0
